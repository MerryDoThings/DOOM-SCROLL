...
## v3.3.0 — C1 + C2 + C3 rimosso + feature varie + maxi review

**C3 — SISTEMA rimosso**
- Eliminati: CSS `.sistema-card`, variabili, `SISTEMA_MSGS` (18 messaggi), funzioni `initSistemaId/formatSistemaMsg/addSistemaCard`, tutti i trigger
- A9 timestamp: convertito da card SISTEMA nel feed a `showToast('il tempo di sessione non corrisponde al tempo percepito.')`

**C1 — metaAware: la consapevolezza che peggiora le cose**
- Dopo una card Lercio auto-referenziale (contiene "questo feed", "inclusa questa", "doom/scroll" ecc.), `metaAware = true` per 2 load
- Durante metaAware: frequenza Lercio dimezzata — il feed "sa che sai" e sommerge di satira

**C2 — anti-bot + debug mode**
- Overlay `🤖 SISTEMA / verifica anti-bot / sei umano?` appare una volta tra load 5-7 (prob 50%)
- **Sì** → niente. **No** → `debugMode` per 2-3 load
- In debug mode: titoli mostrano formula (`[TITOLO_ALLARMANTE]... [FONTE_CREDIBILE]`), fonti → `ALGORITMO_FEED_v2.4` / `ENGAGEMENT_OPTIMIZER_3.1`, tag → `[PAURA_MEDIO_ALTA]` / `[OUTRAGE_BAIT]`
- Poi torna normale con toast "modalità debug terminata."

**Brainrot post card**
- Immagine onirica 1:1 come post Instagram nel feed quando `brainrotLevel >= 2`
- Header: `il.tuo.algoritmo` con avatar gradiente, azioni, like, caption `average brainrot user experience`
- Appare nel finally del batch — visibile mentre scorri

**Sfondo lobotomia QI < 5**
- Immagine anatomica vintage embedded (57KB) come div `position:fixed` a opacità 7-15%
- Transizione 2s, si intensifica con ogni QI perso sotto 5, sparisce al restart

**Easter egg lobotomy**
- Card Lercio con hint appare una volta tra load 8-12 (prob 40%)
- Comando `lobotomy` digitato → -50 QI istantaneo

**Maxi review pre-lancio — bug fixati**
- `localStorage.setItem` in `unlockFinal` e `doomscroll_played` protetti con try/catch
- Toast cap: rimozione immediata invece di delay 320ms — elimina finestra di 3 toast visibili
- `pendingBrainrotImg`: variabile morta rimossa
- Tutti i `sistemaUserId` refs rimossi

**Finale segreto testato online ✅**
